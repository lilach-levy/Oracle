CREATE OR REPLACE PACKAGE STR_UTIL_PKG is
  ...
  -- Function: is_phone
  --
  -- Description: Check if string holds numeric or legal phone characters values.
  --------------------------------------------------------------
  FUNCTION is_phone(p_str VARCHAR2) RETURN VARCHAR2;
...
-- Function: is_phone
--------------------------------------------------------------
FUNCTION is_phone(p_str VARCHAR2)
RETURN VARCHAR2 IS
   l_letter           VARCHAR2(1);
   l_num              NUMBER;
   l_phone_characters VARCHAR2(100) := fnd_profile.value('CNS_PHONE_CHARACTERS');
...
BEGIN
   ...
   LOOP
      ...
      BEGIN
         ...
         l_letter := substr(p_str, i, 1);
         l_num    := to_number(l_letter);

      EXCEPTION
         WHEN OTHERS THEN
            IF instr(l_phone_characters, l_letter) = 0
            THEN
               RETURN 'N';
            END IF;
      END;
